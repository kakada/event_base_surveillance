EBS.MilestonesNew = do ->
  ...
  onClickAddField = ->
    $('form .add_milestone_attributes').off('click')
    $('form .add_milestone_attributes').on 'click', (event) ->
      appendField(this)
      event.preventDefault()

  onClickAddFieldOption = ->
    $(document).off('click', 'form .add_milestone_attribute_options')
    $(document).on 'click', 'form .add_milestone_attribute_options', (event) ->
      appendField(this)
      initMiniColorPicker()
      event.preventDefault()
  ...
  showBtnAddSelectOption = (dom)->
    ...
    optionsWrapper.show()
    optionsWrapper.find('.add_milestone_attribute_options').click()
